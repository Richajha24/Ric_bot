#start
#dictionary of all the data that the bot needs to function
#first category chapter 1 
#@chapterf_name = "The Fundamental unit of life"

data = {
    "chapter_5": {
        "name": "The Fundamental Unit of Life",
        "topics": {
            "5.1": {
                "title": "What are Living Organisms Made Up of?",
                "qa": [
                    {
                        "question": "What are cells?",
                        "answer": [
                            "Cells are the basic units of life.",
                            "They can be unicellular (single-celled) or multicellular (many-celled)."
                        ]
                    },
                    {
                        "question": "What did the discovery of magnifying lenses lead to?",
                        "answer": [
                            "The discovery of magnifying lenses led to the development of microscopes.",
                            "This further led to the discovery of the microscopic world.",
                            "Robert Hooke discovered cells in 1665 using a primitive microscope."
                        ]
                    },
                    {
                        "question": "Who discovered the cell?",
                        "answer": [
                            "Cells were first discovered by Robert Hooke in 1665.",
                            "He observed cork using a primitive microscope."
                        ]
                    },
                    {
                        "question": "Who discovered free-living cells in pond water?",
                        "answer": [
                            "Leeuwenhoek (1674) observed free-living cells in pond water."
                        ]
                    },
                    {
                        "question": "Who discovered the nucleus?",
                        "answer": [
                            "Robert Brown discovered the nucleus in 1831."
                        ]
                    },
                    {
                        "question": "Who coined the term protoplasm?",
                        "answer": [
                            "Purkinje coined the term 'protoplasm' in 1839."
                        ]
                    },
                    {
                        "question": "Who proposed the cell theory?",
                        "answer": [
                            "Schleiden (1838) and Schwann (1839) proposed the cell theory.",
                            "They stated that all plants and animals are composed of cells.",
                            "They also stated that the cell is the basic unit of life."
                        ]
                    },
                    {
                        "question": "Who added that all cells arise from pre-existing cells?",
                        "answer": [
                            "Virchow (1855) stated that all cells arise from pre-existing cells."
                        ]
                    },
                    {
                        "question": "What enabled detailed study of cell structure?",
                        "answer": [
                            "The discovery of the electron microscope in 1940 enabled detailed study of cell structure and organelles."
                        ]
                    },
                    {
                        "question":"What are the two main types of cells?",
                        "answer":[
                            "The two main types of cells are prokaryotic and eukaryotic cells.",
                            "Prokaryotic cells lack a nucleus and membrane-bound organelles, while eukaryotic cells have a nucleus and membrane-bound organelles.",
                            "Prokaryotic cells are generally smaller and simpler than eukaryotic cells.",


                        ]

                    },
                    {
                        "question":"What are the difference between prokaryotic and eukaryotic cells?",
                        "answer":[
                            "Prokaryotic cells lack a nucleus and membrane-bound organelles, while eukaryotic cells have a nucleus and membrane-bound organelles.",
                            "Prokaryotic cells are generally smaller and simpler than eukaryotic cells.",
                            "Prokaryotic cells include bacteria and archaea, while eukaryotic cells include plants, animals, fungi, and protists.", 
                            "Prokaryotic cells reproduce asexually through binary fission, while eukaryotic cells can reproduce both sexually and asexually.",
                            "Prokaryotic cells have a single circular chromosome, while eukaryotic cells have multiple linear chromosomes.",
                            "Prokaryotic cells have a cell wall made of peptidoglycan, while eukaryotic cells may or may not have a cell wall, and if present, it is made of cellulose (in plants) or chitin (in fungi).",
                        ]
                    },
                    {
                        "question":"What are the characteristics of prokaryotic cells?",
                        "answer":[
                            "Prokaryotic cells lack a nucleus and membrane-bound organelles.",
                            "They have a single circular chromosome.",
                            "They reproduce asexually through binary fission.",
                            "They have a cell wall made of peptidoglycan.",
                            "Examples of prokaryotic cells include bacteria and archaea."
                        ]
                    },
                    {
                        "question":"What are the characteristics of eukaryotic cells?",
                        "answer":[
                            "Eukaryotic cells have a nucleus and membrane-bound organelles.",
                            "They have multiple linear chromosomes.",
                            "They can reproduce both sexually and asexually.",
                            "They may or may not have a cell wall, and if present, it is made of cellulose (in plants) or chitin (in fungi).",
                            "Examples of eukaryotic cells include plants, animals, fungi, and protists."
                        ]
                    },
                    {
                        "question":"What are the similarities between prokaryotic and eukaryotic cells?",
                        "answer":[
                            "Both prokaryotic and eukaryotic cells have a plasma membrane, cytoplasm, and ribosomes.",
                            "Both types of cells carry out essential life processes such as metabolism, growth, and reproduction.",
                            "Both prokaryotic and eukaryotic cells can have flagella or cilia for movement."
                        ]
                    },
                    {
                        "question":"What are the basic functions carried out by cells?",
                        "answer":[
                            "Cells carry out essential life processes such as metabolism, growth, reproduction, and response to stimuli.",
                            "They maintain homeostasis by regulating internal conditions.",
                            "They transport materials in and out of the cell through their plasma membrane.",
                            "They produce energy through cellular respiration.",
                            "They synthesize proteins and other necessary molecules."
                        ]
                    },
                    {
                        "question":"How do cells perform basic functions?",
                        "answer":[
                            "There is a division of labor seen within a single cell.",
                            " In fact, each such cell has got certain specific components within it known as cell organelles.",
                            " Each kind of cell organelle performs a special function, , such as makingnew material in the cell, clearing up the waste material from the cell and so on.",
                            "A cell is able to live and perform all its functions because of these organelles.",
                            "These organelles together constitute the basic unit called the cell. It is interesting that all cells are found to have the same organelles, no matter what their function is or what organism they are found in."

                        ]
                    },
                    {
                        "question":"Who discovered the cell and how?",
                        "answer":[
                            "Cells were first discovered by Robert Hooke in 1665.",
                            "He observed cork using a primitive microscope and noticed small compartments which he called 'cells'.",
                            "Later, Leeuwenhoek observed free-living cells in pond water in 1674, and Robert Brown discovered the nucleus in 1831.",
                        ]
                    },
                    {
                        "question":"Why is cell called the structural and functional unit of life?",
                        "answer":[
                            "A cell is called the structural and functional unit of life because it is the smallest unit that can carry out all the functions necessary for life.",
                            "Cells provide structure to organisms, as they are the building blocks of all living things.",
                            "Cells perform essential functions such as metabolism, growth, reproduction, and response to stimuli, making them the functional unit of life."
                        ]
                    }
                ]
            },
            "5.2": {
                "title": "What is a Cell Made Up of? What is the Structural Organisation of a Cell?",
                "qa":[
                   {
                     "question":"What do we see when we look at a cell under a microscope?",
                    "answer":[
                        "If we study a cell under a microscope, we would come across three features in almost every cell; plasma membrane, nucleus and cytoplasm. All activities inside the cell and interactions of the cell with its environment are possible due to these features."
                    ]
                   }
                ]           
            },
            "5.2.1": {
                "title":"Plasma Membrane Or Cell Membrane",
                "qa":[
                    {
                        "question":"What is the plasma membrane?",
                        "answer":[
                            "The plasma membrane, also known as the cell membrane, is a thin, flexible barrier that surrounds the cell.",
                            "It is made up of a phospholipid bilayer with embedded proteins.",
                            "The plasma membrane regulates the movement of substances in and out of the cell, allowing for selective permeability."
                        ]
                    },
                    {
                        "question":"What are the functions of the plasma membrane?",
                        "answer":[
                            "The plasma membrane serves several important functions:",
                            "1. It acts as a protective barrier, separating the internal environment of the cell from the external environment.",
                            "2. It regulates the movement of substances in and out of the cell, allowing for selective permeability.",
                            "3. It facilitates communication between cells through receptor proteins.",
                            "4. It helps maintain the cell's shape and provides structural support."
                        ]
                    }
                ]
            }
        }
    }
}
